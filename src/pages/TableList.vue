...
export default {
  ...
  data() {
    ...
    showFinalizationModalColapso: false,
      ...
      totalVuelos: 0,
      fechaHoraInicio: "",
      totalEnviosColapso: 0,
      totalPaquetesColapso: 0,
      fechaHoraInicioColapso: "",

      totalVuelosColapso: 0,
...
destinationPoint(point, angle, distance) {
  ...
  return [newLng, newLat];
    ...
    async finalizarSimulacionSolo() {
      try {
        const response = await axios.get(urlBase + '/api/simulacion/colapso/finalizarST');
  ...
  fetchAeropuertos() {
    ...
    updateAirportData() {
      ...
      axios.get(urlBase +`/api/simulacion/colapso/aeropuertos?fecha=${fecha}&hora=${hora}`)
        .then(response => {
          this.aeropuertos = response.data;
          this.geojsonAeropuertos.features = this.aeropuertos.map(a => ({
            type: 'Feature',
   ...
   //   console.log(`ENTRO CONDICIONAL ${new Date(nuevoVueloStartTimeLlegada).toISOString()}`);
    ...
    async iniciarPlanificacion() {
      ...
      try {
        ...
        cont = 1;
        const response = await axios.get(urlBase +'/api/simulacion/colapso/iniciarST', {
          params: {
            fecha: fechaInicio,
...
let progressInterval = setInterval(async () => {
  const statusResponse = await axios.get(urlBase+'/api/simulacion/colapso/estado');
  ...
  if (statusResponse.data.progreso === 1.0 || statusResponse.data.estado.includes("Terminado")) {
    ...
    startSimulationLoop(fechaInicio, fechaInicioHora) {
      ...
      initialExecution = false; // Asegura que solo se ejecute una vez
            ...
            this.simulationDateTime = new Date(this.simulationDateTime.getTime() + 60000); // Avanzar 1 hora en tiempo simulado
            this.updateCurrentDateTimeDisplay(); 
    ...
    async continuarSimulacion(fecha, hora) {
      ...
      try {
        const response = await axios.get(urlBase + '/api/simulacion/colapso/continuarST', {
          params: {
            fecha: fecha,
            hora: hora,
            separarPaquetes: separarPaquetesCont,
...
closeFinalizationModal() {
    ...
    async checkSimulationStatus() {
      try {
        const response = await axios.get(urlBase + '/api/simulacion/colapso/estado');
    ...
    async fetchSimulationResults(fecha, hora) {
      try {
        const response = await axios.get(urlBase + '/api/simulacion/colapso/resultados');
...
*/
      ...
      const realTimeSeconds = flightDurationSimulationSeconds / 60;  // Convert to real time seconds based on 360 simulated seconds = 1 real second


     // console.log(flightDurationMinutes)
      const steps = 100; // MEJORA VISULAZCION PERO SE DESFASA ALGO EL TIEMPO PERO SE VE MAS RAPIDO
  ...
  const clickedVuelo = allFlights.find(v => v.id === vueloId);
    ...
    //    console.log(`Buscando aeropuertos con cadena: ${cadena}, fecha: ${fecha}, hora: ${hora}`);
        const response = await axios.get(urlBase + `/api/simulacion/colapso/aeropuerto?cadena=${cadena}&fecha=${fecha}&hora=${hora}`);
  ...
  async buscarEnvio() {
    ...
    //    console.log(`Buscando envios con cadena: ${cadena}, fecha: ${fecha}, hora: ${hora}`);
        const response = await axios.get(urlBase + `/api/simulacion/colapso/paquete?fecha=${fecha}&hora=${hora}&cadena=${cadena}`);
  ...
  async buscarVuelo() {
    if (this.busquedaVuelo.trim() !== '') {
      ...
      if (tipo) {
        try {
          const response = await axios.get(urlBase + '/api/simulacion/colapso/vuelo', {
            params: {
              cadena: cadena,
              tipo: tipo,
              fecha: fecha
